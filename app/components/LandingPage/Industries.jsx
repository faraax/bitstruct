...
export default function Industries() {
    return (
        <div className='bg-[#FAFCFF] py-7 x-container flex flex-col gap-5' id='industries'>
            ...
            <div className='grid 3xl:grid-cols-4 2xl:grid-cols-4 xl:grid-cols-3 lg:grid-cols-2 md:grid-cols-2 sm:grid-cols-1 xs:grid-cols-1 grid-cols-1 gap-5'>
                {
                    cards.map((item) => (
                        <div key={item.id} className='flex flex-col gap-4 items-center'>
                            <div
                                className='industries-card relative object-contain aspect-video w-full h-full'
                            >
                                <Image
                                    src={item.src}
